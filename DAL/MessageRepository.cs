...
namespace DAL
{
    ...
    {
        public MessageRepository(Models.DatabaseContext databaseContext)
            : base(databaseContext)
        {
        }

        public string MetMessageByRequestId(Guid requestid)
        {
            var varMessages =
                Get()
                .Include(x => x.User)
            ...
            string RequestMessage = "[پیامی برای این درخواست در سیستم ثبت نشده است]";
            ...
            if (varMessages.Count > 0)
            {
                ...
                foreach (var message in varMessages)
                {
                    ...
                    //RequestMessage += " - ";
                    //RequestMessage += "<span style='font-weight:bold'>قبل : ";
                    //RequestMessage += EnumValue(Enums.EnumTypes.RequestStates, message.LastState);
                    //RequestMessage += " --> بعد : ";
                    //RequestMessage += EnumValue(Enums.EnumTypes.RequestStates, message.NewState) + "</span>";
